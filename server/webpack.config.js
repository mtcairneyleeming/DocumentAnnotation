...
var fs = require("fs");
var srcPath = path.join(__dirname, '/app'), distPath = path.join(__dirname, '/wwwroot');
...
var bundles = fs.readdirSync(path.join(__dirname, '/app/js/bundles'))
    .map(function (value) { return value.substring(0, value.length - 3); });
var config = {
    cache: true,
    devtool: 'source-map',
    context: srcPath,
    entry: __assign({ 'polyfills': [
            'core-js',
        ], 'jquery': [
        ...
        ] }, bundles.reduce(function (map, value) {
    ...
    ].concat(bundles.filter(function (x) { return x.startsWith("page-"); }).map(function (value) {
    ...
    }), bundles.filter(function (x) { return x.startsWith("page-"); }).map(function (value) {
        return new HtmlWebpackPlugin({
